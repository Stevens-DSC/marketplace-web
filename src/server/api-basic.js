...
const functions = {
    ...
    getAllProducts: async function() {
        ...
        if(!res.ok) {
            throw "Could not fetch products. Got error code " + status
        }

        const json = await res.json()

        if(!json.result) {
            throw "Could not fetch products. Reason: " + (json.reason || json.result || "Not found")
        }
        ...
        if(result.length < 1) {
            throw "Could not fetch products. Reason: Not found"
        }
